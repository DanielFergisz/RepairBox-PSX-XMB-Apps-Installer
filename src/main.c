...
#define PROGRAM_TITLE "RepairBox.pl PSX XMB Dynamic App Installer v1.0"
...
static int usb_storage_available(void)
{
...
static void wait_for_usb(psx_revision_t revision)
{
    unsigned int elapsed = 0;

    while (!usb_storage_available() && elapsed <= USB_WAIT_TIMEOUT_MS) {
        ui_begin();
        ui_printf(PROGRAM_TITLE "\n%s\n\n", revision_title(revision));
        ui_inverse_status("WAITING FOR USB STORAGE");
        ui_printf("Connect the USB storage device.\n\n");
        ui_printf("Elapsed: %u / %u ms\n", elapsed, USB_WAIT_TIMEOUT_MS);
        ui_sync();
                        ...
                        unsigned int completed)
{
    ...
    if (completed == install_count && install_count != 0) {
        ...
        ui_printf("Run v1.0 again with the same id to repair/update.\n");
    }
                          ...
                          const system_version_result_t *version)
{
    ...
    draw_loading(revision, "SCANNING USB APPLICATION FOLDERS");
    ...
    for (;;) {
        ...
        if ((state == PAD_STATE_STABLE || state == PAD_STATE_FINDCTP1) &&
            ...
            if (!finished && hidden_uninstall_chord(current, pressed)) {
                ...
                draw_loading(revision, "RESCANNING USB APPLICATIONS");
                wait_for_usb(revision);
                app_scan_catalog(&catalog);
                draw_catalog(revision, version, &catalog);
